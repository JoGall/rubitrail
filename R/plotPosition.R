...
#' @export
rubitPlotPosition <- function(m, n_radials = 1, n_slices = 1, scale = 1, thigmo_dist = NA, radius = NA, n_bootstraps = 20) {
	...
	if(attributes(m)$tags.hasEnoughPoints) {
		...
		if(!is.na(radius)) {
			...
			if(rad0 * 1.05 < radius) { # account for 3% variation in radius size
